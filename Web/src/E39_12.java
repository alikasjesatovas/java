...
/**
 * Servlet implementation class E39_12
 */
@WebServlet("/E39_12")
public class E39_12 extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		list.add(0, startString);
		list.add(list.size() - 1, endString);
		...
		for (int k = 0; k < list.size(); k++) {
			...
			for (int i = 0; i < s.length() - 1; i++) {
				if (s.substring(i, i + 2).equals("//")) {
					s = s.replaceAll("<b><font color = \"#8904B1\">", "");
					s = s.replaceAll("</font></b>", "");
